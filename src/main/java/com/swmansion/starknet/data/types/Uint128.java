...
public class Uint128 extends NumAsHexBase implements ConvertibleToCalldata {
    ...
    public Uint128(@NotNull BigInteger value) {
        ...
        if (this.getValue().compareTo(BigInteger.ZERO) < 0) {
                throw new IllegalArgumentException("Default Uint128 constructor does not accept negative numbers, [" + this.getValue() + "] given.");
        } else if (this.getValue().compareTo(MAX) >= 0) {
                throw new IllegalArgumentException("Default Uint128 constructor does not accept numbers higher than 2^128-1, [" + this.getValue() + "] given.");
        }
    }
